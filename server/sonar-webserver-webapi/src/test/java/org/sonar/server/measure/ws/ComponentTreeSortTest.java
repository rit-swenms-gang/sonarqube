...
class ComponentTreeSortTest {
    ...
    void setUp() {
        ...
        metrics = newArrayList(violationsMetric, sqaleIndexMetric, newViolationsMetric, reliabilityIssueMetric,
        ...
        for (ComponentDto component : components) {
            measuresByComponentUuidAndMetric.put(component.uuid(), violationsMetric,
                    createFromMetricValue(violationsMetric, currentValue));
            measuresByComponentUuidAndMetric.put(component.uuid(), newViolationsMetric,
                    createFromMetricValue(newViolationsMetric, currentValue));
            measuresByComponentUuidAndMetric.put(component.uuid(), sqaleIndexMetric,
                    createFromMetricValue(sqaleIndexMetric, currentValue));
            measuresByComponentUuidAndMetric.put(component.uuid(), reliabilityIssueMetric,
                    createFromMetricValue(reliabilityIssueMetric, buildJsonImpact((int) currentValue)));
            measuresByComponentUuidAndMetric.put(component.uuid(), newReliabilityIssueMetric,
                    createFromMetricValue(newReliabilityIssueMetric, buildJsonImpact((int) currentValue)));
            currentValue--;
        }
    }
    ...
    void sort_by_names() {
        ...
        assertThat(result).extracting("name")
                .containsExactly("name-1", "name-2", "name-3", "name-4", "name-5", "name-6", "name-7", "name-8",
                        "name-9");
    }
    ...
    void sort_by_qualifier() {
        ...
        assertThat(result).extracting("qualifier")
                .containsExactly("qualifier-9", "qualifier-8", "qualifier-7", "qualifier-6", "qualifier-5",
                        "qualifier-4",
    ...
    void sort_by_numerical_metric_key_ascending() {
        components.add(
                newComponentWithoutSnapshotId("name-without-measure", "qualifier-without-measure",
                        "path-without-measure"));
    ...
    void sort_by_numerical_metric_key_descending() {
        ...
        assertThat(result).extracting("path")
                .containsExactly("path-9", "path-8", "path-7", "path-6", "path-5", "path-4", "path-3", "path-2",
                        "path-1",
                        "path-without-measure");
    }
    ...
    void sort_by_alert_status_ascending() {
        ...
        for (int i = 0; i < components.size(); i++) {
            ...
            measuresByComponentUuidAndMetric.put(component.uuid(), metrics.get(0),
                    createFromMetricValue(metrics.get(0), alertStatus));
        }
        ComponentTreeRequest wsRequest = newRequest(newArrayList(METRIC_SORT, NAME_SORT), true,
    ...
    void sort_by_numerical_metric_period_1_key_descending() {
        components.add(
                newComponentWithoutSnapshotId("name-without-measure", "qualifier-without-measure",
                        "path-without-measure"));
        ComponentTreeRequest wsRequest = newRequest(singletonList(METRIC_PERIOD_SORT), false, NEW_METRIC_KEY)
                .setMetricPeriodSort(1);
    ...
    void sort_by_numerical_metric_period_1_key_ascending() {
        components.add(
                newComponentWithoutSnapshotId("name-without-measure", "qualifier-without-measure",
                        "path-without-measure"));
        ComponentTreeRequest wsRequest = newRequest(singletonList(METRIC_PERIOD_SORT), true, NEW_METRIC_KEY)
                .setMetricPeriodSort(1);
    ...
    void sortComponent_whenMetricIsNewAndMetricPeriodSort_shouldOrderByTotal() {
        components.add(
                newComponentWithoutSnapshotId("name-without-measure", "qualifier-without-measure",
                        "path-without-measure"));
        ComponentTreeRequest wsRequest = newRequest(singletonList(METRIC_PERIOD_SORT), false,
                NEW_DATA_IMPACT_METRIC_KEY)
                .setMetricPeriodSort(1);
    ...
    void sortComponent_whenMetricIsNotNewAndMetricPeriodSort_shouldNotOrder() {
        components.add(
                newComponentWithoutSnapshotId("name-without-measure", "qualifier-without-measure",
                        "path-without-measure"));
        ComponentTreeRequest wsRequest = newRequest(singletonList(METRIC_PERIOD_SORT), false, DATA_IMPACT_METRIC_KEY)
                .setMetricPeriodSort(1);

        List<ComponentDto> result = sortComponents(wsRequest);

        assertThat(result).extracting("path")
                .containsExactly("path-9", "path-7", "path-8", "path-6", "path-3", "path-4", "path-5", "path-1",
                        "path-2",
                        "path-without-measure");
    }
    ...
    private static ComponentTreeRequest newRequest(List<String> sortFields, boolean isAscending,
